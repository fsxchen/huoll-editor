...
type Persisted = {
  ...
  /** taskId → back-end articleId map so sync survives refresh. */
  ...
  persist(
    (set, get) => ({
      ...
      newTask: (init) => {
        ...
        // if backend articleId was provided, remember the mapping
        if (init?.articleId) {
          set((s) => ({ articleIdMap: { ...s.articleIdMap, [t.id]: init.articleId! } }));
        }
        return t.id;
      },
      deleteTask: (id) => {
    ...
    }),
    {
      ...
      migrate: (persisted, fromVersion): Persisted => {
        ...
        if (fromVersion < 8 && persisted && typeof persisted === "object") {
          ...
          if (!p.publishApiKey) p.publishApiKey = process.env.NEXT_PUBLIC_PUBLISH_API_KEY ?? "";
        }
        // v8 → v9: add articleIdMap for backend sync
        if (fromVersion < 9 && persisted && typeof persisted === "object") {
          const p = persisted as Record<string, unknown>;
          if (!p.articleIdMap || typeof p.articleIdMap !== "object") {
            p.articleIdMap = {};
          }
        }
