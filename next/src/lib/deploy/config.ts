 ...
 * `~/.huoll-editor/<provider>.json` (chmod 600). When the API surfaces them
 ...
 * Acceptable because (a) Huoll Editor is a developer tool the user runs
...
export function deployConfigPath(providerId: DeployProviderId): string {
  // `HUOLL_EDITOR_STATE_DIR` lets tests / sandboxed environments
  // redirect storage. Otherwise fall back to `~/.huoll-editor/`.
  const base =
    process.env.HUOLL_EDITOR_STATE_DIR ||
    process.env.HTML_ANYTHING_USER_STATE_DIR ||
    path.join(homedir(), ".huoll-editor");
  const file =
