 ...
 * tree collapsed away — huoll-editor tasks are always a single
...
// huoll-editor's whole UX is "share this link" — opt the freshly created
...
export function safeVercelProjectName(raw: string): string {
  return (
    safeProjectLabel(raw, 80) || `huoll-editor-${randomUUID().slice(0, 8)}`
  );
}
...
export async function deployToVercel({
  ...
  const createResp = await fetch(
    `${VERCEL_API}/v13/deployments${vercelTeamQuery(config)}`,
    {
      ...
      body: JSON.stringify({
        name: safeVercelProjectName(`huoll-editor-${taskId}`),
        files: files.map((f) => ({
          file: f.file,
