 ...
 * and tag top-level children with data-tool="huoll-editor" so WeChat trusts the styles.
...
export function toWechatHtml(fullHtml: string): string {
  ...
  Array.from(wrap.children).forEach((child) => {
    child.setAttribute("data-tool", "huoll-editor");
  });
  ...
  return `<section data-tool="huoll-editor">${inlined}</section>`;
}
