...
import { useImagePrompt } from "@/lib/use-image-prompt";
...
export function EditorPane() {
  ...
  const { run: runImagePrompt, cancel: cancelImagePrompt, status: imagePromptStatus, result: imagePromptResult, error: imagePromptError } = useImagePrompt();
  const t = useT();
      ...
      >
        ...
        {hasContent && (
          <div className="flex flex-wrap items-center justify-end gap-x-2.5 gap-y-1 whitespace-nowrap text-[11px] text-[var(--ink-faint)]">
            ...
            <button
              onClick={() => {
                if (imagePromptStatus === "running") {
                  cancelImagePrompt();
                } else {
                  runImagePrompt(content).then((result) => {
                    // Insert result as a markdown comment block at the end
                    const store = useStore.getState();
                    const prev = store.tasks.find((x) => x.id === store.activeTaskId)?.content ?? "";
                    const comment = `\n\n<!-- 配图提示词 -->\n<!--\n${result.trim()}\n-->\n`;
                    store.setContent(prev + comment);
                  });
                }
              }}
              disabled={!content.trim()}
              className="shrink-0 whitespace-nowrap text-[10px] uppercase tracking-[0.14em] text-[var(--ink-faint)] hover:text-[var(--ink)] transition-colors disabled:opacity-40"
              title={imagePromptStatus === "running" ? "停止生成" : "根据内容生成配图提示词"}
            >
              {imagePromptStatus === "running" ? "🛑 停止" : "🎨 配图"}
            </button>
            <button
              onClick={() => {
                const store = useStore.getState();
                ...
                store.setContent(prev + sep + "![图片描述](图片URL)\n");
              }}
              className="shrink-0 whitespace-nowrap text-[10px] uppercase tracking-[0.14em] text-[var(--ink-faint)] hover:text-[var(--ink)] transition-colors"
              title="插入图片占位符"
            >
