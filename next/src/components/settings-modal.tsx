...
function PublishSection() {
  ...
  useEffect(() => {
    const envUrl = process.env.NEXT_PUBLIC_PUBLISH_API_URL ?? "";
    const envKey = process.env.NEXT_PUBLIC_PUBLISH_API_KEY ?? "";
    if (!publishApiUrl && envUrl) setPublishApiUrl(envUrl);
    if (!publishApiKey && envKey) setPublishApiKey(envKey);
  }, []);
  ...
  const testConnection = async () => {
    ...
    setTestResult(null);
    try {
      const res = await fetch(`${url.replace(/\/+$/, "")}/articles/?page_size=1`, {
