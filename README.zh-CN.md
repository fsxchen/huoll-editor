...
<p align="center"><sub>来自 <a href="https://github.com/nexu-io/open-design"><b>Open Design</b></a> 团队 —— <b>40k★ · 200+ 贡献者</b>,更生产级、迭代更快。huoll-editor 是聚焦在 agent 时代 HTML 编辑器这一刀的专项; 如果你喜欢这个味道, 同一拨人做的 <a href="https://github.com/nexu-io/open-design">Open Design</a> 是它在更大规模上的形态, 顺手也看看。</sub></p>
...
| **部署** | 本地 `pnpm -F huoll-editor dev` / Vercel 一键部署 web 层（agent 永远跑在本地）。 |
| **License** | Apache-2.0 |
...
- `next/` 是完整 Next 应用（`huoll-editor`）。
- `e2e/` 是浏览器测试包（`@huoll-editor/e2e`），也是 Playwright 用例的唯一真相源。
...
pnpm -F huoll-editor dev
pnpm -F huoll-editor typecheck
pnpm -F huoll-editor test
pnpm -F huoll-editor build
pnpm -F @huoll-editor/e2e typecheck
pnpm -F @huoll-editor/e2e test
```
...
| 部署 | 本地 `pnpm -F huoll-editor dev` · Vercel 一键 web 层（agent 永远跑本地） |
...
- **新 skill** —— 在 [`next/src/lib/templates/skills/`](next/src/lib/templates/skills/) 加一个文件夹,带 `SKILL.md` + `example.html`(可选 `assets/` 和 `references/`)。`pnpm -F huoll-editor dev` 重启后 picker 自动发现。`SKILL.md` frontmatter 必须设 `mode` · `scenario` · `surface` · `preview` · `design_system` —— 抄一个邻居改就行。
